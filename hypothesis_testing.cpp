...
static const string ONE_VAR_FILE="data/oneVariable_dataset.txt";
static const string TWO_VAR_FILE="data/twoVariable_dataset.txt";
static const string ANOVA_FILE="data/anova.txt";
...
static vector<double> readOneVariable()
{
    vector<double> x;
    ifstream in(ONE_VAR_FILE);
    if(!in){cout<<"Cannot open "<<ONE_VAR_FILE<<endl;return x;}
    double v;
...
static void readTwoVariable(vector<double> &x,vector<double> &y)
{
    ifstream in(TWO_VAR_FILE);
    if(!in){cout<<"Cannot open "<<TWO_VAR_FILE<<endl;return;}
    double v1,v2;
    while(in>>v1>>v2){x.push_back(v1);y.push_back(v2);}
    in.close();
}
...
double z_valueFromTable(double value)
{
    ...
    if(!file.is_open()){cout<<"Cannot open Z_table.csv"<<endl;return -1;}
    ...
    while(getline(file,line))
    {
        ...
        else
        {
            ...
            vector<double> row;
            for(size_t i=1;i<vals.size();i++) row.push_back(stod(vals[i]));
            table.push_back(row);
        }
    }
    file.close();
    ...
    for(int i=0;i<(int)baseValues.size();i++)
        if(fabs(baseValues[i]-baseZ)<eps){rowIdx=i;break;}
    for(int i=0;i<(int)increments.size();i++)
        if(fabs(increments[i]-inc)<eps){colIdx=i;break;}
    if(rowIdx!=-1&&colIdx!=-1) return table[rowIdx][colIdx];
...
double t_valueFromTable(int dof,double alpha)
{
    ...
    if(!file.is_open()){cout<<"Cannot open T_table.csv"<<endl;return -1;}
    ...
    while(getline(file,line))
    {
        ...
        if(firstRow)
        {
            for(size_t i=1;i<vals.size();i++) sigLevels.push_back(stod(vals[i]));
            firstRow=false;
        }
        else
        {
    ...
    for(int i=0;i<(int)dofs.size();i++)
        if(dofs[i]==dof){rowIdx=i;break;}
    for(int i=0;i<(int)sigLevels.size();i++)
        if(fabs(sigLevels[i]-alpha)<1e-9){colIdx=i;break;}
    if(rowIdx!=-1&&colIdx!=-1) return table[rowIdx][colIdx];
...
double chi_valueFromTable(double dof,double alpha)
{
    ...
    if(!file.is_open()){cout<<"Cannot open chi_square_table.csv"<<endl;return -1;}
    vector<double> sigLevels;
    vector<int> dofs;
    vector<vector<double>> table;
    string line;
    ...
    int rowIdx=-1,colIdx=-1;
    for(int i=0;i<(int)dofs.size();i++)
        if(dofs[i]==(int)dof){rowIdx=i;break;}
    for(int i=0;i<(int)sigLevels.size();i++)
        if(fabs(sigLevels[i]-alpha)<1e-9){colIdx=i;break;}
    if(rowIdx!=-1&&colIdx!=-1) return table[rowIdx][colIdx];
...
{
    vector<double> x=readOneVariable();
    if(x.empty()) return;
    ...
    bool reject=false;
    if(sign=='>'&&z_stat>z_crit) reject=true;
    if(sign=='<'&&z_stat<-z_crit) reject=true;
    if(reject)
...
void oneSided_Ttest(double null_hypothesis,char sign)
{
    ...
    bool reject=false;
    if(sign=='>'&&t_stat>t_crit) reject=true;
    if(sign=='<'&&t_stat<-t_crit) reject=true;
    if(reject)
        cout<<"  Decision: REJECT H0 (mean = "<<null_hypothesis<<")"<<endl;
    else
...
void oneWayANOVA()
{
    ifstream in(ANOVA_FILE);
    if(!in){cout<<"Cannot open "<<ANOVA_FILE<<endl;return;}
    ...
    while(getline(in,line))
    {
        ...
        while(ss>>v)
        {
            if(idx>=(int)groups.size()) groups.resize(idx+1);
    ...
    in.close();
    int k=groups.size();
    if(k<2){cout<<"Need at least 2 groups for ANOVA."<<endl;return;}
    int N=0;
    for(int i=0;i<k;i++) N+=groups[i].size();
    double grandSum=0;
    for(int i=0;i<k;i++)
        for(double v:groups[i]) grandSum+=v;
